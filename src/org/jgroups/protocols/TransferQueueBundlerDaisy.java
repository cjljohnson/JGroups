 ...
 */
public class TransferQueueBundlerDaisy extends BaseBundler implements Runnable {
    ...
    public TransferQueueBundlerDaisy() {
        this.remove_queue=new ArrayList<>(16);
    }

    protected TransferQueueBundlerDaisy(BlockingQueue<Message> queue) {
        this.queue=queue;
        this.remove_queue=new ArrayList<>(16);
    }

    public TransferQueueBundlerDaisy(int capacity) {
    ...
    public TransferQueueBundlerDaisy removeQueueSize(int size) {this.remove_queue=new ArrayList<>(size); return this;}
    ...
    protected void sendSingleMessage(final Message msg) {
        ...
        try {
            ...
            if (dest == null && next != null) {
                //System.out.println("Daisy Transport");
                transport.doSend(output.buffer(), 0, output.position(), next);
            } else {
